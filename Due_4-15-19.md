...
1. What is the procedure for logging into Hoffman2?
	I am using the google chrome secure shell extension. To log into Hoffman, you click the shell icon and open the tab which is called 
	[your username]@hoffman2.idre.ucla.edu. Then, you just give your password.
2. What is the first thing that you do once you login?  What is the command?
	The first thing you should do is to get a node. The command is "qrsh".
...
1. You can use two flags at the same time. What does the command ls do when used with the -l flag? What about if you use both the -l and the -h flag?
	When you use the "-l" flag, it adds more information about file sizes and dates edited, and lists the files vertically. If you
	use both -l and -h, it prints the same information in a human-readable format.
2. The command ls -R lists the contents of directories recursively, i.e., lists their sub-directories, sub-sub-directories, and so on at each level. The command ls -t lists things by time of last change, with most recently changed files or directories first. In what order does ls -R -t display things? Hint: ls -l uses a long listing format to view timestamps.
	ls -R -t orders the directories by the date they were modified, and then lists the contents of each directory.
...
2. what is ../
	It's the directory above your current one.
3. what is ./?
	It's your current durectory.
...
````
She could use "cd ~", "cd ~/data/..", or "cd .." only.

$ cd .
	This will not move her, since it will move her into the directory she's currently in (no change).
$ cd /
	This will move her into the "/" directory, which is not the home directory.
$ cd /home/amanda
	This will move her into the /home/amanda directory, which is not her home directory.
$ cd ../..
	This will take her 2 directories above her current directory, which is not her home directory.
$ cd ~
	This will take her to the home directory.
$ cd home
	This will do nothing, because you need "$" before home for this command to work.
$ cd ~/data/..
	This will take her to the home directory, because it will take her to home, then go into data, then back up to home.
$ cd
	This will do nothing because there is no directory specified.
$ cd ..
	This will take her up a directory, which is her home directory.
````
---
### File systems
...
```
 What did the touch command do? When you look at your current directory using the GUI file explorer, does the file show up?
	The touch command creates an empty text file named "my_file.txt". Yes, the file shows up.
 Use 'ls -l' to inspect the files. How large is my_file.txt?
	The file is 0B.
 When might you want to create a file this way?
	You might want to create a file this way if you were organizing them to add information into them later.
 Can you use touch to edit my_file.txt?
	No, touch will not edit a file.
...
Hint: Try executing each command twice in a row and then examining the output files.
	">" overwrites the file with the information, whereas ">>" appends this information to the file.
...
```
	I didn't have permission to create the animals-subset file. But the answer would be "c".
...
```
$ mv ../analysed/sucrose.dat  ../analyzed/maltose.dat ./
```
...
4. What happens when we execute `rm -i thesis_backup/quotations.txt`? Why would we want this protection when using rm?
	It asks for confirmation that you really want to delete this file. This is useful with rm because rm will permanently
	remove files from the directory.
5. In the example below, what does cp do when given several filenames and a directory name?
	cp will copy those two files into the directory "backup"
```
$ mkdir backup
...
$ cp amino-acids.txt animals.txt morse.txt
	It says that morse.txt is not a directory.
```
...
```
	She needs to run "$ mv fructose.dat sucrose.dat analyzed/".
...
```
	The commands that would achieve this objective are sets 1 and 2 (I labeled them below). Set 3 would not successfully make any files unless you 
	added the -p tag. Set 4 would make the directory 2016-05-20 which contains the directories data, raw, and processed, which is not the stucture
	we want.
...
Explain why -n has this effect.
	-n has this effect because it tells the computer to sort the entries numerically. Otherwise, it tries to sort them alphabetically which
	doesn't give the desired results with numbers.
...
```
	The command "wc -l * | sort -n | head -n 3" would work.
...
Why do you think uniq only removes adjacent duplicated lines? (Hint: think about very large data sets.) What other command could you combine with it in a pipe to remove all duplicated lines?
	Because in a large data set, there might be a duplicated line farther into the document which is under a different category, and which still
	represents unique data, not just a duplication. To remove all duplicated lines, you could first sort the data and then use the pipe to run uniq
	on that result.
...
Hint: build the pipeline up one command at a time to test your understanding
	The text below will be stored in final.txt:
2012-11-06,deer
2012-11-06,rabbit
2012-11-05,raccoon
...
What other command(s) could be added to this in a pipeline to find out what animals the file contains (without any duplicates in their names)?
	You could add the sort and uniq commands in a pipeline. The command would look like this:
...
```
	You would use "cut -d, -f 2 animals.txt | sort | uniq -c".
...
Can you match the same set of files with basic wildcard expressions that do not use the [ ] syntax? Hint: You may need more than one expression.
	You can use "*A.txt *B.txt" to group the same files.
The expression that you found and the expression from the lesson match the same set of files in this example. What is the small difference between the outputs?
	I see no difference. They give me exactly the same result when I use them with ls on this data. 
Under what circumstances would your new expression produce an error message where the original one would not?
	If there were no files ending in one of the expressions (ex. no files ended in B.txt), then the new expression would give an error, where the
...
```
	I don't know what the "/" is supposed to do. The software gives me an error if I type that in the commands. The command that would work is
	"rm *.txt".
